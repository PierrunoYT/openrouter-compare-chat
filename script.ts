...
class ChatUI {
    ...
    private currentStreamControllers: Map<string, AbortController>;
    ...
    private async fetchAvailableModels(): Promise<void> {
        try {
            ...
            this.availableModels = data.data.sort((a: Model, b: Model) => {
    ...
    private filterModels(searchTerm: string): void {
        ...
        this.modelCheckboxes.querySelectorAll('.provider-header, .model-checkbox').forEach((element) => {
            if (element.classList.contains('provider-header')) {
                ...
                currentProvider = (element as HTMLElement).dataset.provider || '';
    ...
    private createModelCheckboxes(): void {
    ...
    private setupEventListeners(): void {
        ...
        this.modelSearch.addEventListener('input', (e: Event) => {
            const searchTerm = (e.target as HTMLInputElement).value;
            this.filterModels(searchTerm);
        });

        this.modelCheckboxes.addEventListener('change', (e: Event) => {
        ...
        this.userInput.addEventListener('dragover', (e: DragEvent) => {
            e.preventDefault();
            e.stopPropagation();
        ...
        this.userInput.addEventListener('drop', async (e: DragEvent) => {
            e.preventDefault();
            e.stopPropagation();
            this.userInput.classList.remove('dragover');
        ...
        const parameterInputs = document.querySelectorAll('[data-param]') as NodeListOf<HTMLInputElement>;
        parameterInputs.forEach(input => {
            const param = input.dataset.param as keyof ModelParameters;
            input.addEventListener('change', () => {
                const value = parseFloat(input.value);
                if (!isNaN(value)) {
                    (this.modelParameters[param] as number) = value;
                }
            });
        });
    }
    ...
    private async sendMessageToModels(message: Message): Promise<void> {
        this.messages.push(message);
        this.addMessage('user', typeof message.content === 'string' ? message.content : 'Sent image with message: ' + (message.content[0] as MessageContent).text);
        ...
        try {
            ...
            if (error instanceof OpenRouterError) {
                if (error.code === 403 && error.metadata) {
                    const moderationError = error.metadata as ModerationErrorMetadata;
    ...
    private async streamResponse(message: Message, modelId: string, messageGroup: HTMLElement): Promise<void> {
            ...
            while (true) {
                ...
                for (const line of lines) {
                    if (line.startsWith('data: ') && line !== 'data: [DONE]') {
                        try {
                            const data = JSON.parse(line.slice(6)) as OpenRouterResponse;
                            const content = (data.choices[0] as StreamingChoice)?.delta?.content || '';
                            if (content) {
                                hasContent = true;
    ...
    private addCacheInfo(modelId: string, cacheDiscount: number, messageGroup: HTMLElement): void {
    ...
    private createResponseDiv(modelId: string, messageGroup: HTMLElement): HTMLDivElement {
    ...
    private addModelResponse(modelId: string, content: string, messageGroup: HTMLElement, isError: boolean = false): void {
        const modelName = this.availableModels.find(m => m.id === modelId)?.name || modelId;
